//paper-check-pro-front\src\pages\PaperCnInfoCompletion\index.js
import React, { useState } from 'react';
import { Upload, message, Card, Spin,Descriptions,Flex ,Button, Typography,Space} from 'antd';
import { InboxOutlined } from '@ant-design/icons';
import { useDispatch, useSelector } from 'react-redux';
import { setPaperCnPdfCmpInfo ,setPaperCnInfo} from '@/store/modules/papercnStore';
import { request } from '@/utils'; // 导入配置好的axios实例
import { getPaperCnAnalysis } from '@/apis/papercn';
...
const PaperCnInfoCompletion = () => {
    const navigate = useNavigate();
        const dispatch = useDispatch();
        // 从redux获取存储的论文信息
        const {papercninfo, papercnpdfcmpinfo} = useSelector(state => state.papercn);
    ...
    const handleUpload = async ({ file }) => {
        ...
        try {
            // 1. 上传解析PDF
            const uploadRes = await request.post('/api/PaperInfoQuery/paper_query/paper_cn/pdf/analysis', formData, {
                headers: {
            ...
            if (!uploadRes.data) {
             ...
             //对比存储的论文标题和上传的PDF标题
                 if (uploadRes.data.title !== papercninfo.title) {
                   message.warning('上传的PDF标题与查询的论文标题不一致,请检查！');
                   return;
            } 
            // 2. 发起数据对比请求
            const analysisRes = await getPaperCnAnalysis(uploadRes.data, papercninfo);

            if (analysisRes.data) {
                dispatch(setPaperCnPdfCmpInfo(analysisRes.data.paper_info));
                dispatch(setPaperCnInfo(analysisRes.data.paper_info));
                          ...
                          >
                            <Flex justify="space-between" align="center">
                              ...
                              <Button 
                                ...
                                onClick={() => navigate('/papercn/enhancement',
                                    { state: { 
                                        paperinfonoenhan:papercnpdfcmpinfo
                                    } }
                                )}
                              >
                                开始分析
...
export default PaperCnInfoCompletion;