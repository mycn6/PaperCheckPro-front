// src/pages/PaperInfoEditor/index.js
import React, { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { Form, Input, Button, Select, Card, Space, InputNumber } from 'antd';
import { useNavigate } from 'react-router-dom';
import { setPaperEnEditorInfo } from '@/store/modules/paperStore';

const { Option } = Select;

const PaperInfoEditor = () => {
  const [form] = Form.useForm();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const paperInfo = useSelector(state => state.paper.paperinfo);
  ...
  const handleSubmit = (values) => {
    ...
    dispatch(setPaperEnEditorInfo(processedData));
    navigate(-1);
  };
  ...
  return (
    <div style={{ padding: 24, maxidth: 1200, margin: '0 auto' }}>
      <Card title="论文信息编辑">
        <Form
          form={form}
        ...
        paddingBottom: 16 // 底部留白
                ...
                >
                  {affiliations.map((aff, affIndex) => (
                    aff && <Option key={affIndex} value={affIndex+1}>
                     机构 {affIndex + 1}: {aff}
                    </Option>
                  ))}
                </Select>
...
export default PaperInfoEditor;